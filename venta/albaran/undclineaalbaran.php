...
if($guardar) {
    ...
    if($guardar == 'U') {
        ...
        $id=intval($env_csv)+1;
	$sql="INSERT INTO $tabla (Id, Producto, Stock, Venta, Cantidad, Descuento, Precio, IVA, Descripcion) VALUES ($id,'$producto','$stock','$albaran','$cantidad','$descuento','$precio','$iva','$descripcion')";
        $result = mysqli_query($con,$sql);
...
$sql="SELECT AlbLineas.Venta, Producto.Producto, AlbLineas.Cantidad, AlbLineas.Descuento, AlbLineas.Precio AS PrecioBase, AlbLineas.Precio*(1-AlbLineas.Descuento/100)*AlbLineas.Cantidad AS Total , AlbLineas.Precio*(1-AlbLineas.Descuento/100)*(1+AlbLineas.IVA/100)*AlbLineas.Cantidad as PrecioFinal FROM AlbLineas INNER JOIN Producto ON AlbLineas.Producto=Producto.Id WHERE AlbLineas.Venta = '".$albaran."'";
