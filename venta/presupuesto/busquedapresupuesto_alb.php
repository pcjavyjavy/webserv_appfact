...
$sql="SELECT Presupuesto.Id, Cliente.Nombre, Cliente.Apellidos, Presupuesto.NPres, Presupuesto.Fecha, Presupuesto.Validez, ROUND(SUM(PresLineas.Cantidad*PresLineas.Precio*(1-PresLineas.Descuento/100)*(1+PresLineas.IVA/100)),2) as Total FROM Presupuesto INNER JOIN PresLineas ON Presupuesto.Id=PresLineas.Presupuesto INNER JOIN Cliente ON Presupuesto.Cliente=Cliente.Id WHERE Presupuesto.Albaran=0 AND Cliente.Id $cliente AND Presupuesto.NPres LIKE '%$presupuesto%' AND Presupuesto.Fecha BETWEEN '$minfecha' AND '$maxfecha' AND Presupuesto.Validez BETWEEN '$vminfecha' AND '$vmaxfecha' GROUP BY Presupuesto.Id ORDER BY Presupuesto.Fecha DESC, Cliente.Nombre, Cliente.Apellidos";
