...
cursor.execute("SELECT Albaran.NAlb, Albaran.Fecha, Albaran.Notas, Cliente.Nombre, Cliente.Apellidos, Cliente.DNI, Cliente.Direccion, Cliente.Cp, Cliente.Poblacion, Cliente.Provincia, Cliente.Mail FROM Albaran INNER JOIN Cliente ON Albaran.Cliente=Cliente.Id WHERE Albaran.Id=%s", (IDFra,))
...
PosLR=30
Color=ColorCabecera
comando="convert -font "+Fuente+" -fill "+Color+" -pointsize "+str(Tamano)+" -draw \"text "+str(PosLR)+","+str(LineaActual)+" '"+str('Albaran N.: '+ClienteNFra+'          Fecha: '+ClienteFecha)+"'\" "+WorkingFile+" "+WorkingFile
os.system(comando)
LineaActual+=20
...
cursor.execute("SELECT ROUND(AlbLineas.Cantidad,2) AS Cantidad, Producto.Producto, Producto.Descripcion AS DescrPro, AlbLineas.Descripcion AS DescrVent, ROUND(AlbLineas.Precio,2) AS Precio, ROUND(AlbLineas.Descuento,1) AS Descuento, ROUND(AlbLineas.IVA,1) AS IVA FROM AlbLineas INNER JOIN Producto ON AlbLineas.Producto=Producto.Id WHERE AlbLineas.Venta=%s", (IDFra,))
...
ClienteAsunto=str(EmpresaNombre.decode("latin-1").encode("utf-8"))+" Albaran A"+str(ClienteNFra)
EmpresaAsunto="Albaran A"+str(ClienteNFra)+" Cliente "+str(ClienteNombre.decode("latin-1").encode("utf-8"))+" "+str(ClienteApellidos.decode("latin-1").encode("utf-8"))
#Debug
...
if len(sys.argv)==3:
    if sys.argv[2]=='C':
        if not ClienteMail == '':
            ...
            comando="sudo mutt -s \""+ClienteAsunto+"\" -a "+OutputFile+" -- "+ClienteMail+" < mensajea"
            os.system(comando)
    elif sys.argv[2]=='A':
        if not ClienteMail == '':
            #Mandar correo al cliente
            ...
            os.system(comando)
        #Mandar correo a mi
        comando="sudo mutt -s \""+EmpresaAsunto+"\" -a "+OutputFile+" -- "+EmpresaMail+" < mensajeam"
        os.system(comando)
    elif sys.argv[2]=='Y':
        #Mandar correo a mi
        comando="sudo mutt -s \""+EmpresaAsunto+"\" -a "+OutputFile+" -- "+EmpresaMail+" < mensajeam"
