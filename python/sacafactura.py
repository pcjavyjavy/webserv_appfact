...
cursor.execute("SELECT FVenta.NFra, FVenta.Fecha, FVenta.Notas, Cliente.Nombre, Cliente.Apellidos, Cliente.DNI, Cliente.Direccion, Cliente.Cp, Cliente.Poblacion, Cliente.Provincia, Cliente.Mail FROM FVenta INNER JOIN Cliente ON FVenta.Cliente=Cliente.Id WHERE FVenta.Id=%s", (IDFra,))

for NFra, Fecha, Notas, Nombre, Apellidos, DNI, Direccion, Cp, Poblacion, Provincia, Mail in cursor:
...
PosLR=30
Color=ColorCabecera
comando="convert -font "+Fuente+" -fill "+Color+" -pointsize "+str(Tamano)+" -draw \"text "+str(PosLR)+","+str(LineaActual)+" '"+str('Factura N.: '+ClienteNFra+'          Fecha: '+ClienteFecha)+"'\" "+WorkingFile+" "+WorkingFile
os.system(comando)
LineaActual+=20
...
cursor.execute("SELECT ROUND(Venta.Cantidad,2) AS Cantidad, Producto.Producto, Producto.Descripcion AS DescrPro, Venta.Descripcion AS DescrVent, ROUND(Venta.Precio,2) AS Precio, ROUND(Venta.Descuento,1) AS Descuento, ROUND(Venta.IVA,1) AS IVA FROM Venta INNER JOIN Producto ON Venta.Producto=Producto.Id WHERE Venta.Venta=%s", (IDFra,))
...
for Cantidad, Producto, DescrPro, DescrVent, Precio, Descuento, IVA in cursor:
    ...
    if LineaActual+max(LongitudProducto,LongitudDescripcion)*10 >= LineasSuma-EntreSecciones:
...
if ClienteNotas== ' ':
    ...
    if LineaActual+LongitudNotas*10 >= LineasSuma-35:
        # TODO VALORES HASTA EL MOMENTO AL FINAL DE PAGINA
        sumafinal.SumandoVoy(PosicionamientoSuma, LineasSuma, WorkingFile, Tamano, Fuente, ColorSumas, SumaCantidades, SumaPrecioBase, SumaDto, SumaIVA, SumaPrecioTotal)
        Contador+=1
        WorkingFile=str(MarcaFecha)+str(Contador)+WorkingName+WorkingExt
...
ClienteAsunto=str(EmpresaNombre.decode("latin-1").encode("utf-8"))+" Factura F"+str(ClienteNFra)
EmpresaAsunto="Factura F"+str(ClienteNFra)+" Cliente "+str(ClienteNombre.decode("latin-1").encode("utf-8"))+" "+str(ClienteApellidos.decode("latin-1").encode("utf-8"))
#Debug
...
if len(sys.argv)==3:
    if sys.argv[2]=='C':
        if not ClienteMail == '':
            ...
            comando="sudo mutt -s \""+ClienteAsunto+"\" -a "+OutputFile+" -- "+ClienteMail+" < mensajef"
            os.system(comando)
    elif sys.argv[2]=='A':
        if not ClienteMail == '':
            #Mandar correo al cliente
            ...
            os.system(comando)
        #Mandar correo a mi
        comando="sudo mutt -s \""+EmpresaAsunto+"\" -a "+OutputFile+" -- "+EmpresaMail+" < mensajefm"
        #print comando
        #print "\n"
        os.system(comando)
    elif sys.argv[2]=='Y':
